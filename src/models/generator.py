...
class Block(nn.Module):
    def __init__(
        ...
        # Jeśli down=False, robimy konwolucję transponowaną (powiększanie w dekoderze)
        self.conv = (
            nn.Conv2d(
                in_channels,
                out_channels,
...
class Generator(nn.Module):
    def __init__(self, in_channels=3, features=64):
        ...
        # ------------------------------------------
        # Pierwsza warstwa zazwyczaj nie ma BatchNorm, więc robimy ją ręcznie
        ...
        # Wąskie gardło (Bottleneck) na samym dnie U-Netu
        ...
        # UWAGA: in_channels to features*8 * 2, ponieważ doklejamy dane ze skip connections!
        self.up2 = Block(
        ...
        # Ostatnia warstwa wracająca do 3 kanałów (RGB) i rozmiaru 256x256
        self.final_up = nn.Sequential(
            nn.ConvTranspose2d(features * 2, 3, kernel_size=4, stride=2, padding=1),
            nn.Tanh(),  # Tanh normalizuje piksele do przedziału [-1, 1], co jest standardem w GANach
        )
    ...
    def forward(self, x):
        ...
        # Przepuszczamy przez dekoder, używając torch.cat do DOKLEJANIA zapisanych warstw
        up1 = self.up1(bottleneck)
        up2 = self.up2(torch.cat([up1, d7], dim=1))  # dim=1 to łączenie po kanałach
